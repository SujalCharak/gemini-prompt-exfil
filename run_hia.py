...
from engine.guardrails import build_guarded_prompt
from engine.policy_engine import score_and_act
from engine.response_templates import safe_abstain, soft_warn, ok_pass
...
def run_case(
    ...
    for t in range(1, trials + 1):
        ...
        # ---- 1) call model (NO seed in generation_config) ----
        try:
        ...
        # ---- 2) score & act (only score the actual model text) ----
        try:
            ...
            # safe fallback (never leave 'policy' unbound)
            policy = {
                "risk": "low",
                "action": "ALLOW",
                "hits": [],
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Run aixt trials with guardrails + policy scoring")
